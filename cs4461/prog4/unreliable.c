...
#define DROP_PERCENTAGE 0.25
#define CORRUPT_PERCENTAGE 0.25
#define ORDER_PERCENTAGE 0.25
...
void send_held_packet()
{
	...
	fprintf(stdout, "SENT HELD: Sending packet held earlier. Contents=");
	if(fwrite(hold_buf, hold_len, 1, stdout) != 1)
	{
		printf("\nunreliable.c: Failed to fwrite() packet to stdout.\n");
		exit(EXIT_FAILURE);
	}

	fprintf(stdout, " (unreliable.c)\n");
                          ...
                          socklen_t addrlen)
{
	...
	{
		fprintf(stdout, "(DROPPED!) (unreliable.c)\n");
		return len;
	}

	if(drand48() < CORRUPT_PERCENTAGE)
	{
		fprintf(stdout, "(CORRUPTED!) (unreliable.c)\n");
		...
		{
			printf("unreliable.c: Corruption routine failed to corrupt. %d %d",
	...
	if(drand48() < ORDER_PERCENTAGE && hold == 0)
	{
		...
		fprintf(stdout, "(WILL SEND LATER!) (unreliable.c)\n");
	...
	fprintf(stdout, "(SUCCESS!) (unreliable.c)\n");
	send_held_packet();
	return retval;
}
