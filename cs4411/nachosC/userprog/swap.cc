...
Swap::Swap(SpaceId sid, int pageNum){
  ...
  fileSystem->Create(const_cast<char*>(swapFileName.c_str()), pageNum * PageSize);
  swapFile = fileSystem->Open(const_cast<char*>(swapFileName.c_str()));
...
Swap::~Swap(){
  delete swapFile;
  fileSystem->Remove(const_cast<char*>(swapFileName.c_str()));
}
